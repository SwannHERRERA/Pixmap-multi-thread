...
int main(int argc, char **argv) {
    // Check the number of arguments (only one expected)
    assert(argc == 2 ? 1 : !fprintf(stderr, "Wrong number of arguments!\n"));
    ...
    assert(!pthread_create(&thread1, NULL, &ppm_black_pixels_T1, (void *) &arguments) ? 1 : !fprintf(stderr,
                                                                                                     "Cannot create new thread : T1\n"));

    // Creating thread 2
    // Calling function ppm_black_pixels_T2
    // Passing the address of the "arguments" structure, casted as void*
    assert(!pthread_create(&thread2, NULL, &ppm_black_pixels_T2, (void *) &arguments) ? 1 : !fprintf(stderr,
                                                                                                     "Cannot create new thread : T2\n"));

    // Waiting for T1 to finish
    assert(!pthread_join(thread1, NULL) ? 1 : !fprintf(stderr, "Cannot wait for thread 1 to finish.\n"));

    // Waiting for T2 to finish
    assert(!pthread_join(thread2, NULL) ? 1 : !fprintf(stderr, "Cannot wait for thread 1 to finish.\n"));
